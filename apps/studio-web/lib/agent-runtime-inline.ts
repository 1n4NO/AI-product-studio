/**
 * Inlined subset of @product-studio/agent-runtime used by API routes.
 *
 * Vercel builds studio-web in isolation and cannot resolve the local monorepo
 * package. All code here is copied verbatim from packages/agent-runtime/src/.
 */
...
export class AuditLogStore {
  ...
  async append(
    input: Omit<AuditLogEntry, "id" | "timestamp" | "previousHash" | "hash">
    ...
    const hash: string = sha256(JSON.stringify(entryBase));
    const entry: AuditLogEntry = { ...entryBase, hash };

    await this.ensureRoot();
    await appendFileAtomic(path.join(this.rootDir, LOG_FILE_NAME), `${JSON.stringify(entry)}\n`);
    await this.writeIndex({ lastHash: hash, count: index.count + 1 });
  ...
  async verifyIntegrity(): Promise<{ ok: boolean; brokenAtId?: string }> {
    ...
    for (const entry of entries) {
      if (entry.previousHash !== previousHash) return { ok: false, brokenAtId: entry.id };
      const recalculated = sha256(
        JSON.stringify({
          id: entry.id, timestamp: entry.timestamp, correlationId: entry.correlationId,
          actor: entry.actor, action: entry.action, resourceType: entry.resourceType,
          resourceId: entry.resourceId, metadata: entry.metadata, previousHash: entry.previousHash,
        })
      );
      if (recalculated !== entry.hash) return { ok: false, brokenAtId: entry.id };
  ...
  async exportEntries(): Promise<{
    ...
    return { exportedAt: new Date().toISOString(), count: entries.length, entries, integrity };
  }

  async enforceRetention(retainDays: number): Promise<{ removed: number; remaining: number }> {
    if (retainDays < 1) throw new Error("retainDays must be >= 1");
    const allEntries = await this.list(100000);
    if (allEntries.length === 0) return { removed: 0, remaining: 0 };

    const cutoff   = Date.now() - retainDays * 24 * 60 * 60 * 1000;
    const retained = allEntries.filter((e) => {
    ...
    const rebuilt = retained.map((entry) => {
      const base = {
        id: entry.id, timestamp: entry.timestamp, correlationId: entry.correlationId,
        actor: entry.actor, action: entry.action, resourceType: entry.resourceType,
        resourceId: entry.resourceId, metadata: entry.metadata, previousHash,
      };
    ...
    await this.ensureRoot();
    const content = rebuilt.map((e) => JSON.stringify(e)).join("\n");
    await writeFile(
      path.join(this.rootDir, LOG_FILE_NAME),
      content ? `${content}\n` : "",
      "utf8"
    );
